...
session_start();
	...
	if(isset($_REQUEST['login']))
	{
		...
		{
			$msg='unm';
			header("location:v_login.php?res=$msg");			
		...
		if($row = mysql_fetch_array($result))
			  ...
			  if($a=='')
			  {
				  header("location:frgtpass.php?id1=$result[0]");
			   } //exit();
			 $query="insert into log_history(`c_name`,`login_time`)values('".$_POST['username']."','".$tim."')";
			 mysql_query($query);
			 header("Location:frgtpass.php?id=$row[0]");
		  	}
		 else
			{
			$msg='unk';
...
<div id="login-box-name">Password:</div><div id="login-box-field"><input name="password" type="password" class="form-login" title="Password" placeholder="Enter Password" size="30" maxlength="2048" />
...
<br />
<span class="login-box-options"> <a href="frgtpass.php" style="margin-left:30px;">Forgot password?</a></span>
</form>
</div>
